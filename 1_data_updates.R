...
# merge with site data to get fire year
...
## double Fire Years, make updates
...
#######################################################
## Add in Heat Load Index for Spring and each transect
#######################################################

#McCune and Keon, 2022:
...
# DEM in ArcGIS using the extract surface information tool when field aspect data was missed.
...
## There are other missing columns, but we will deal with these later.


# ### creating canopy cover index
# #I assigned arbitrary weights to each canopy cover class. Higher index values align with higher canopy cover density
# transectdata$canopy_cover_index <- with(transectdata, 0.25 * (CC_1_PCT/100) + 0.5 * (CC_2_PCT/100) + 0.75 * (CC_3_PCT/100))


#### 5 year post fire SPEI ####
## import SPEI data derived from gridMET, downloaded from climatetoolbox.org
# SPEI_data = read.csv("C:/Users/gpeven/OneDrive - University of Idaho/Springs Research/Data/Climate/gridmetDROUGHT_SPEI_1yr.csv")
# 
# 
# # Filter to only include growing season months
# # to test for seasonal water deficits
# monthly_SPEI <- SPEI_data %>%
#   filter(Month %in% c(5, 6, 7, 8, 9))
# 
# # Group by "Year" and calculate the average seasonal spring/summer SPEI for each year
# seasonal_SPEI <- monthly_SPEI  %>%
#   group_by(Year) %>%
#   summarise(Avg_SPEI = mean(SPEI))
# 
# 
# ### Average SPEI over 5 years after each fire
# 
# SPEI_Golden = seasonal_SPEI %>%
#   filter(Year %in% c("1989", "1990", "1991", "1992", "1993")) %>%
#   summarize(Avg_SPEI_5yr = mean(Avg_SPEI))
# 
# SPEI_Golden$Fire_Year = 1988
# 
# SPEI_Diamond = seasonal_SPEI %>%
#   filter(Year %in% c("2001", "2002", "2003", "2004", "2005")) %>%
#   summarize(Avg_SPEI_5yr = mean(Avg_SPEI))
# 
# SPEI_Diamond$Fire_Year = 2000
# 
# 
# SPEI_Goat = seasonal_SPEI %>%
#   filter(Year %in% c("2007", "2008", "2009", "2010", "2011")) %>%
#   summarize(Avg_SPEI_5yr = mean(Avg_SPEI))
# 
# SPEI_Goat$Fire_Year = 2006
# 
# SPEI_fires = rbind(SPEI_Golden, SPEI_Diamond, SPEI_Goat)
# transectdata <- left_join(transectdata, SPEI_fires, by = "Fire_Year") 
# 
# #### Temp and Precip Data
# 
# temp_precip = read.csv("C:/Users/gpeven/OneDrive - University of Idaho/Springs Research/Data/Climate/gridmet_TWRS_1981_2023_temp_precip.csv")
# 
# # Filter to only include growing season months
# # to test for seasonal water deficits
# monthly_tp <- temp_precip %>%
#   filter(Month %in% c(5, 6, 7, 8, 9))
# 
# # Group by "Year" and calculate the average seasonal spring/summer SPEI for each year
# seasonal_tp<- monthly_tp  %>%
#   group_by(Year) %>%
#   summarise(Avg_tmin_degC = mean(tmin_degC), Avg_tmax_degC = mean(tmax_degC), avg_precip = mean(precip_mm), sum_precip = sum(precip_mm))
# 
# 
# ### Average SPEI over 5 years after each fire
# ## Fields: average 5 yr t min, average 5 yr t max, average growing season precip over 5 yrs, average cumulative precip over 5 years
# 
# TP_Golden = seasonal_tp%>%
#   filter(Year %in% c("1989", "1990", "1991", "1992", "1993")) %>%
#   summarize(Avg_tmin_degC_5yr = mean(Avg_tmin_degC), Avg_tmax_degC_5yr = mean(Avg_tmax_degC), avg_precip_5yr = mean(avg_precip), avg_sum_precip_5yr = mean(sum_precip))
# 
# TP_Golden$Fire_Year = 1988
# 
# TP_Diamond = seasonal_tp %>%
#   filter(Year %in% c("2001", "2002", "2003", "2004", "2005")) %>%
#   summarize(Avg_tmin_degC_5yr = mean(Avg_tmin_degC), Avg_tmax_degC_5yr = mean(Avg_tmax_degC), avg_precip_5yr = mean(avg_precip), avg_sum_precip_5yr = mean(sum_precip))
# 
# TP_Diamond$Fire_Year = 2000
# 
# 
# TP_Goat = seasonal_tp %>%
#   filter(Year %in% c("2007", "2008", "2009", "2010", "2011")) %>%
#   summarize(Avg_tmin_degC_5yr = mean(Avg_tmin_degC), Avg_tmax_degC_5yr = mean(Avg_tmax_degC), avg_precip_5yr = mean(avg_precip), avg_sum_precip_5yr = mean(sum_precip))
# 
# TP_Goat$Fire_Year = 2006
# 
# TP_fires = rbind(TP_Golden, TP_Diamond, TP_Goat)
# transectdata <- left_join(transectdata, TP_fires, by = "Fire_Year") 


###############################################
## 5. Create new dataframe with density data ##
###############################################
...
#####################################
## Apply Age equations to all data ##
####################################
...
# ######################################################################
# ########################################################
# ## Let's try splitting data up into <=10 years post fire and >= 10 years 
# ## post fire to see if there's a better relationship to post-fire climate
# #########################################################
# transectdata$est_yrs_since_fire = (transectdata$Years_Since_Fire) - (transectdata$Age_new)
# 
# tree_counts_age = 
#   transectdata %>%
#   group_by(Distance_range_m, Distance_rank, Species, Spring_Name, Latitude, 
#            Longitude,Spring_Type, Transect_ID, Forest_Type, Area_m2, Area_ha, Average_dist_to_live_tree, Count,
#            Age_new, Fire_Year) %>%
#   summarize(Tree_Count = sum(Count))
# 
# tree_counts_age$Density = tree_counts_age$Tree_Count/tree_counts_age$Area_ha   
# 
# tree_counts_age_reburn <- tree_counts_age %>%
#   filter(Spring_Name %in% c("West Goat", "Little Hole", "Black Talus", "Equisetum", "Sedge", "Ceanothus", "Bog", "Picea"))
# 
# tree_counts_age_Golden <- tree_counts_age %>%
#   filter(Fire_Year == 1988)
# View(tree_counts_age_Golden)
# View(tree_counts_age_reburn)
# ### Make sure these are factors
# tree_counts_age$Distance_range_m = factor(tree_counts_age$Distance_range_m )
# tree_counts_age$Fire_Year = factor(tree_counts_age$Fire_Year )
# tree_counts_age$Spring_Name = factor(tree_counts_age$Spring_Name )
# tree_counts_age$Transect_ID = factor(tree_counts_age$Transect_ID )
# 
# 
# ############################################
# #####  Standardize Continous Covariates for tree_counts_age ###
# #############################################
# ## need to standardize continous co-variates to all have a comparable values 
# #  and avoid computational issues with code. This doesn't change the value
# #  of the variables, just the scale.
# 
# MyStd <- function(x) { (x - mean(x)) / sd(x)}
# 
# tree_counts_age$Average_dist_to_live_tree.c <- MyStd(tree_counts_age$Average_dist_to_live_tree)
# tree_counts_age$canopy_cover_index.c  <- MyStd(tree_counts_age$canopy_cover_index)
# tree_counts_age$Slope.c  <- MyStd(tree_counts_age$Slope)
# tree_counts_age$HLI_spring.c   <- MyStd(tree_counts_age$HLI_spring)
# tree_counts_age$HLI_transect.c   <- MyStd(tree_counts_age$HLI_transect)
# tree_counts_age$Transect_Slope_deg.c   <- MyStd(tree_counts_age$Transect_Slope_deg)
# tree_counts_age$Years_Since_Fire.c   <- MyStd(tree_counts_age$Years_Since_Fire)
# tree_counts_age$elevation_m.c   <- MyStd(tree_counts_age$elevation_m)
# tree_counts_age$Area_ha.c <- MyStd(tree_counts_age$Area_ha)
# tree_counts_age$Area_m2.c <- MyStd(tree_counts_age$Area_m2)
# tree_counts_age$Distance_rank.c <-MyStd(tree_counts_age$Distance_rank)
# tree_counts_age$Avg_SPEI_5yr.c <-MyStd(tree_counts_age$Avg_SPEI_5yr)
# tree_counts_age$Avg_tmin_degC_5yr.c <-MyStd(tree_counts_age$Avg_tmin_degC_5yr)
# tree_counts_age$Avg_tmax_degC_5yr.c <-MyStd(tree_counts_age$Avg_tmax_degC_5yr)
# tree_counts_age$avg_sum_precip_5yr.c <-MyStd(tree_counts_age$avg_sum_precip_5yr)
# tree_counts_age$avg_precip_5yr.c <-MyStd(tree_counts_age$avg_precip_5yr)
# # ################################
# 
# 
# tree_counts_early = 
#   tree_counts_age %>%
#   filter(est_yrs_since_fire <= 10)
# 
# 
# tree_counts_later = 
#   tree_counts_age %>%
...
############################################
#####  Standardize Continous Covariates for tree_counts ###
#############################################
## need to standardize continous co-variates to all have a comparable values 
#  and avoid computational issues with code. This doesn't change the value
#  of the variables, just the scale.
